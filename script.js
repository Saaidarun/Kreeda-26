...
function processResults(results, pointsConfig, category) {
    ...
    // Process Winner / 1st
    if (results.winner && results.winner.house) {
        const house = results.winner.house.toLowerCase();
        if (houseScores[category].hasOwnProperty(house)) {
            houseScores[category][house] += pointsConfig[1];
        }
    }

    // Process Runner-up / 2nd
    if (results.runnerUp && results.runnerUp.house) {
        const house = results.runnerUp.house.toLowerCase();
        if (houseScores[category].hasOwnProperty(house)) {
            houseScores[category][house] += pointsConfig[2];
        }
    }

    // Process 3rd Place (if applicable)
    if (results.thirdPlace && results.thirdPlace.house && pointsConfig[3]) {
        const house = results.thirdPlace.house.toLowerCase();
        if (houseScores[category].hasOwnProperty(house)) {
            houseScores[category][house] += pointsConfig[3];
        }
    }
}
...
// Old Admin Mode Renamed
function initAdminMode_OLD() {
    ...
    if (savedData) {
        try {
            ...
            // Update the global eventDetails object
            Object.assign(eventDetails, parsedData);
            console.log('✅ Loaded event details from local storage');
        } catch (e) {
    ...
    const adminDataEditor = document.getElementById('adminDataEditor');
    ...
    function openAdminDashboard() {
        ...
        adminDataEditor.value = JSON.stringify(eventDetails, null, 4);
        adminSaveMsg.style.display = 'none';
        adminJsonError.style.display = 'none';
    }
    ...
    if (adminSaveBtn) {
        adminSaveBtn.addEventListener('click', () => {
            try {
                const jsonStr = adminDataEditor.value;
                const newData = JSON.parse(jsonStr);

                // Update global object
                // We clear and assign to keep reference if possible, or just replace content
                // eventDetails is const, so we modify its properties
                for (const key in eventDetails) delete eventDetails[key];
                Object.assign(eventDetails, newData);

                // Save to LocalStorage
                localStorage.setItem('sportsDayEventDetails', JSON.stringify(newData));

                adminJsonError.style.display = 'none';
                adminSaveMsg.style.display = 'block';
                setTimeout(() => adminSaveMsg.style.display = 'none', 3000);

                console.log('✅ Data saved successfully!');
            } catch (e) {
                adminJsonError.style.display = 'block';
                adminJsonError.textContent = 'Invalid JSON: ' + e.message;
            }
        });
    }
    ...
    if (adminResetBtn) {
        adminResetBtn.addEventListener('click', () => {
            if (confirm('Are you sure you want to reset to default data? This will clear your local changes.')) {
                localStorage.removeItem('sportsDayEventDetails');
                location.reload(); // Reload to fetch original JS data
            }
        });
    }
}
